...
app.get('/api/time-logs', async (req, res) => {
    try {
        // Use DATE_FORMAT to return the date as a string directly from MySQL to avoid JS timezone conversion issues
        const [rows] = await db.query("SELECT student_id as id, name, program, year, DATE_FORMAT(date, '%Y-%m-%d') as date, time_in as timeIn FROM time_logs");
        res.json(rows);
    } catch (error) {
